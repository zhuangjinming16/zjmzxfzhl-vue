<template>
    <div class="app-container">
        <div class="filter-container">
            <el-select v-model="listQuery.logType" placeholder="日志类型" class="filter-item">
                <el-option v-for="(item, index) in dicts.logType" :key="index" :label="item.content"
                           :value="item.value"></el-option>
            </el-select>
            <el-input v-model="listQuery.userId" placeholder="操作用户" style="width: 200px;" class="filter-item"
                      @keyup.enter.native="btnQuery"/>
            <el-input v-model="listQuery.clientId" placeholder="客户端ID" style="width: 200px;" class="filter-item"
                      @keyup.enter.native="btnQuery"/>
            ...
            <el-button-group>
                <el-button v-permission="'sys:log:save'" icon="el-icon-plus" type="primary" @click="btnCreate"
                           class="filter-item">新增
                </el-button>
                <el-button v-permission="'sys:log:delete'" icon="el-icon-delete" @click="btnDelete()"
        ...
        >
            ...
            <el-table-column label="日志类型" prop="logType" align="center">
                <template slot-scope="scope"><span v-html="formatDictText(dicts.logType,scope.row.logType)"></span>
                </template>
            </el-table-column>
            <el-table-column label="日志内容" prop="logContent" align="center">
                <template slot-scope="scope"><span>{{ scope.row.logContent }}</span></template>
            </el-table-column>
            <el-table-column label="操作用户" prop="userId" align="center">
                <template slot-scope="scope"><span>{{ scope.row.userId }}</span></template>
            </el-table-column>
            ...
            <el-table-column label="耗时" prop="costTime" align="center">
                <template slot-scope="scope"><span>{{ scope.row.costTime }}</span></template>
            </el-table-column>
                ...
                <template slot-scope="{row}">
                    <el-dropdown>
                        ...
                        <el-dropdown-menu slot="dropdown">
                            ...
                            <el-dropdown-item v-permission="'sys:log:update'" icon="el-icon-edit" divided
                                              @click.native="btnUpdate(row)">修改
                            </el-dropdown-item>
                            <el-dropdown-item v-permission="'sys:log:delete'" icon="el-icon-delete" divided
                                              @click.native="btnDelete(row.logId)">删除
                            </el-dropdown-item>
                        </el-dropdown-menu>
                    </el-dropdown>
                </template>
        ...
        <el-dialog title="系统日志" :visible.sync="dialogFormVisible">
            <el-form ref="dataForm" :rules="rules" :model="temp" :disabled="dialogStatus==='view'"
                ...
                <el-form-item label="UUID" prop="logId">
                    <el-input v-model="temp.logId" :readonly="dialogStatus==='update'"/>
                </el-form-item>
                <el-form-item label="日志类型" prop="logType">
                    <el-select v-model="temp.logType" placeholder="日志类型">
                        <el-option v-for="(item, index) in dicts.logType" :key="index" :label="item.content"
                                   :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item label="操作类型" prop="operateType">
                    <el-input v-model="temp.operateType"/>
                </el-form-item>
                ...
                <el-form-item label="浏览器" prop="userAgent">
                    <el-input v-model="temp.userAgent"/>
                </el-form-item>
                ...
                <el-form-item label="请求参数" prop="requestParam">
                    <el-input v-model="temp.requestParam"/>
                </el-form-item>
                <el-form-item label="请求类型" prop="requestType">
                    <el-input v-model="temp.requestType"/>
                </el-form-item>
                <el-form-item label="操作结果" prop="operateResult">
                    <el-input v-model="temp.operateResult"/>
                </el-form-item>
                <el-form-item label="耗时" prop="costTime">
                    <el-input v-model="temp.costTime"/>
                </el-form-item>
            </el-form>
...
<script>
    ...
    export default {
        ...
        beforeCreate() {
            this.getDicts('logType').then(({data}) => {
                this.dicts = data
            })
        },
        created() {
