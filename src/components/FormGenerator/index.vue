...
export default {
  ...
  methods: {
    ...
    setJSON(json) {
      let formDataTemp = deepClone(json)
      this.drawingList = formDataTemp.fields
      if(this.drawingList && this.drawingList.length){
        this.activeFormItem(this.drawingList[0])
      }
      delete formDataTemp.fields
      this.formConf = formDataTemp
    },
    getJSON() {
      this.AssembleFormData()
